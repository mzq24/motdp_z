...
class AnnealedEnergyGuidancePolicy(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.use_joint_state_diffusion = bool(route_b_cfg.get('use_joint_state_diffusion', False))
        self.joint_state_diffusion_loss_weight = float(
            route_b_cfg.get('joint_state_diffusion_loss_weight', 1.0)
        )
        self.joint_state_label_smoothing = float(
            route_b_cfg.get('joint_state_label_smoothing', 0.05)
        )
        self.joint_state_conflict_area_pos_prob = float(
            route_b_cfg.get('joint_state_conflict_area_pos_prob', 0.95)
        )
        ...
        self.use_conflict_timing_state = bool(route_b_cfg.get('use_conflict_timing_state', False))
        self.conflict_area_status_loss_weight = float(route_b_cfg.get('conflict_area_status_loss_weight', 0.25))
        ...
        model = TransformerForDiffusion(
            ...
            use_joint_state_diffusion=self.use_joint_state_diffusion,
        )
        self.model = model
    ...
    def _build_conflict_area_route_target(
        ...
        *,
        ...
        if offline_mask is not None and offline_valid is not None:
            B = offline_mask.reshape(offline_mask.shape[0], -1).shape[0]
            offline_mask = offline_mask.reshape(B, -1).to(device=device, dtype=model_dtype)
            offline_valid = offline_valid.reshape(B, -1).to(device=device, dtype=model_dtype)
            if offline_mask.shape[1] >= route_steps and offline_valid.shape[1] >= route_steps:
                known = offline_valid[:, :route_steps] >= 0.0
                if known.any():
                    target = offline_mask[:, :route_steps].clamp(0.0, 1.0)
                    area_valid = known & (offline_valid[:, :route_steps] > 0.5)
                    return (target, area_valid) if return_valid_mask else target
        ...
        area_valid = torch.ones_like(target, dtype=torch.bool)
        valid_mask = (
    ...
    def _smooth_distribution(
        ...
        eps = self.joint_state_label_smoothing if eps is None else float(eps)
        K = probs.shape[-1]
        smoothed = probs * (1.0 - eps) + (eps / max(K, 1))
        return smoothed / smoothed.sum(dim=-1, keepdim=True).clamp(min=1e-6)

    @staticmethod
    def _log_prob_target(probs: torch.Tensor) -> torch.Tensor:
        return torch.log(probs.clamp(min=1e-4, max=1.0))

    @staticmethod
    def _logit_prob_target(probs: torch.Tensor) -> torch.Tensor:
    ...
    def _get_conflict_timing_targets(
        ...
        *,
        ...
        if status is None or dist_entry is None or dist_exit is None or time_entry is None:
            if require:
                raise ValueError(
                    "conflict timing state requires conflict_area_status, "
                    "conflict_dist_to_entry_m, conflict_dist_to_exit_m, and conflict_time_to_entry_s"
                )
            return None

        B = family_codes.shape[0]
        status = status.reshape(B).to(device=device).clamp(min=0, max=3)
        dist_entry = dist_entry.reshape(B).to(device=device, dtype=model_dtype)
        dist_exit = dist_exit.reshape(B).to(device=device, dtype=model_dtype)
        time_entry = time_entry.reshape(B).to(device=device, dtype=model_dtype)
        active = family_codes.reshape(B).to(device=device) > 0
        finite = (
            torch.isfinite(dist_entry)
            & torch.isfinite(dist_exit)
            & torch.isfinite(time_entry)
            & active
        )
        dist_scale = max(float(self.conflict_timing_dist_norm_scale), 1e-6)
        time_scale = max(float(self.conflict_timing_time_norm_scale), 1e-6)
        timing_values = torch.stack(
            [
                (dist_entry / dist_scale).clamp(-1.0, 1.0),
                (dist_exit / dist_scale).clamp(-1.0, 1.0),
                (time_entry / time_scale).clamp(0.0, 1.0),
            ],
            dim=-1,
        )
        timing_valid = finite.unsqueeze(-1).expand_as(timing_values)
        timing_values = torch.where(
            timing_valid,
            timing_values,
            torch.zeros_like(timing_values),
        )
        return {
            'status_target': status,
            'timing_values': timing_values,
            'timing_valid_mask': timing_valid,
            'dist_entry_m': dist_entry,
            'dist_exit_m': dist_exit,
            'time_entry_s': time_entry,
        }
    ...
    def _stack_boundary_targets(
        ...
        target_list = [
            self._get_stage1_batch_tensor(batch, name, device=device, model_dtype=model_dtype)
        ...
        valid_mask = torch.stack(valid_list, dim=-1) > 0.5
        return targets, valid_mask

    @staticmethod
    def _stack_boundary_predictions(raw_stage1_scores: dict) -> torch.Tensor:
        return torch.stack(
            [
                raw_stage1_scores['merge_yld_max'],
                raw_stage1_scores['merge_go_min'],
                raw_stage1_scores['chase_max'],
                raw_stage1_scores['junction_yld_max'],
                raw_stage1_scores['junction_go_min'],
                raw_stage1_scores['borrow_yld_max'],
                raw_stage1_scores['borrow_go_min'],
            ],
            dim=-1,
        )

    def _project_raw_state_to_diffusion_state(
        self,
        raw_state: dict,
    ) -> dict:
        projected = {
            ...
            'control_phase_logits': raw_state['control_phase_logits'],
            'conflict_area_logits': raw_state['conflict_area_logits'],
        ...
        if 'conflict_area_status_logits' in raw_state:
            projected['conflict_area_status_logits'] = raw_state['conflict_area_status_logits']
        if 'conflict_timing_values' in raw_state:
            projected['conflict_timing_values'] = raw_state['conflict_timing_values']
        return projected
    ...
    def _build_joint_state_clean_targets(
        ...
        if family_codes is None or dir_codes is None or decision_phase_codes is None or control_phase_codes is None:
            raise ValueError("joint state diffusion requires new conflict family/dir/phase labels")
        ...
        decision_probs = torch.full((B, 2), 0.5, device=device, dtype=model_dtype)
        if decision_active.any():
            decision_probs[decision_active] = F.one_hot(
                (decision_phase_codes[decision_active] - 1).clamp(min=0, max=1), num_classes=2
            ).to(device=device, dtype=model_dtype)

        control_active = control_phase_codes > 0
        control_probs = torch.full((B, 4), 0.25, device=device, dtype=model_dtype)
        if control_active.any():
            control_probs[control_active] = F.one_hot(
                (control_phase_codes[control_active] - 1).clamp(min=0, max=3), num_classes=4
            ).to(device=device, dtype=model_dtype)

        speed_target = self._compute_speed_target(trajectory, device, batch=batch)
        if speed_target is None:
            speed_target = torch.full(
                (B, len(self.model.speed_classes)),
                1.0 / len(self.model.speed_classes),
                device=device,
                dtype=model_dtype,
            )
        else:
            speed_target = speed_target.to(device=device, dtype=model_dtype)

        conflict_area_result = self._build_conflict_area_route_target(
            batch=batch,
        ...
        if conflict_area_result is None:
            conflict_area_target = torch.zeros(B, self.num_waypoints, device=device, dtype=model_dtype)
            conflict_area_valid_mask = torch.ones_like(conflict_area_target, dtype=torch.bool)
        else:
            conflict_area_target, conflict_area_valid_mask = conflict_area_result
        conflict_area_probs = torch.where(
            conflict_area_target > 0.5,
        ...
        conflict_area_clean = self._logit_prob_target(conflict_area_probs)
        conflict_area_clean = torch.where(
            conflict_area_valid_mask,
            conflict_area_clean,
            torch.zeros_like(conflict_area_clean),
        )
        ...
        conflict_timing_targets = None
        if self.use_conflict_timing_state:
            conflict_timing_targets = self._get_conflict_timing_targets(
                batch=batch,
                family_codes=family_codes,
                device=device,
                model_dtype=model_dtype,
        ...
        boundary_targets_norm = (boundary_targets_mps / max(self.stage1_boundary_norm_scale, 1e-6)).clamp(0.0, 1.0)
        boundary_targets_norm = torch.where(
            boundary_valid_mask,
            boundary_targets_norm,
            torch.zeros_like(boundary_targets_norm),
        )
        borrow_time_s = self._get_stage1_borrow_time_target(batch, device=device, model_dtype=model_dtype)

        clean_state = {
            'window_logits': self._log_prob_target(self._smooth_distribution(window_probs)),
            'dir_logits': self._log_prob_target(self._smooth_distribution(dir_probs)),
            'decision_phase_logits': self._log_prob_target(self._smooth_distribution(decision_probs)),
            'control_phase_logits': self._log_prob_target(self._smooth_distribution(control_probs)),
            'speed_logits': self._log_prob_target(self._smooth_distribution(speed_target)),
            'conflict_area_logits': conflict_area_clean,
            'boundary_values': boundary_targets_norm,
        }
        metadata = {
            'window_target': window_target,
            'dir_target': dir_codes.clamp(min=0, max=3),
            'decision_phase_codes': decision_phase_codes,
            'control_phase_codes': control_phase_codes,
            'decision_phase_valid_mask': decision_active,
            'control_phase_valid_mask': control_active,
            'speed_target_probs': speed_target,
            'conflict_area_target': conflict_area_target,
            'conflict_area_valid_mask': conflict_area_valid_mask,
            'boundary_targets_norm': boundary_targets_norm,
        ...
        if conflict_timing_targets is not None:
            status_probs = F.one_hot(
                conflict_timing_targets['status_target'],
                num_classes=4,
            ).to(device=device, dtype=model_dtype)
            clean_state['conflict_area_status_logits'] = self._log_prob_target(
                self._smooth_distribution(status_probs)
            )
            clean_state['conflict_timing_values'] = conflict_timing_targets['timing_values']
            metadata.update({
                'conflict_area_status_target': conflict_timing_targets['status_target'],
                'conflict_timing_values': conflict_timing_targets['timing_values'],
                'conflict_timing_valid_mask': conflict_timing_targets['timing_valid_mask'],
                'conflict_dist_to_entry_m': conflict_timing_targets['dist_entry_m'],
                'conflict_dist_to_exit_m': conflict_timing_targets['dist_exit_m'],
                'conflict_time_to_entry_s': conflict_timing_targets['time_entry_s'],
            })
    ...
    def _ddim_step_state_dict(
        self,
        pred_clean_state: dict,
        noisy_state: dict,
        timestep: int,
    ) -> dict:
        scheduler = self._get_state_scheduler()
    ...
    def _compute_joint_state_recon_loss(
        self,
        pred_state: dict,
        clean_state: dict,
        metadata: dict,
        ...
        decision_mask = metadata['decision_phase_valid_mask'].to(device=device, dtype=torch.bool)
        control_mask = metadata['control_phase_valid_mask'].to(device=device, dtype=torch.bool)
        boundary_valid_mask = metadata['boundary_valid_mask'].to(device=device, dtype=torch.bool)
        ...
        window_recon = F.mse_loss(pred_state['window_logits'], clean_state['window_logits'])
        dir_recon = F.mse_loss(pred_state['dir_logits'], clean_state['dir_logits'])
        decision_recon = _masked_mse(
            pred_state['decision_phase_logits'],
            clean_state['decision_phase_logits'],
            decision_mask,
        )
        control_recon = _masked_mse(
            pred_state['control_phase_logits'],
            clean_state['control_phase_logits'],
            control_mask,
        )
        ...
        if conflict_area_mask is None:
            conflict_area_mask = torch.ones_like(clean_state['conflict_area_logits'], dtype=torch.bool)
        else:
            conflict_area_mask = conflict_area_mask.to(device=device, dtype=torch.bool)
        conflict_area_recon = _masked_mse(
            pred_state['conflict_area_logits'],
            clean_state['conflict_area_logits'],
            conflict_area_mask,
        )
        temp_occ_recon = zero
        ...
        area_status_recon = zero
        if 'conflict_area_status_logits' in clean_state and 'conflict_area_status_logits' in pred_state:
            area_status_recon = F.mse_loss(
                pred_state['conflict_area_status_logits'],
                clean_state['conflict_area_status_logits'],
            )
        conflict_timing_recon = zero
        if 'conflict_timing_values' in clean_state and 'conflict_timing_values' in pred_state:
            timing_valid_mask = metadata['conflict_timing_valid_mask'].to(device=device, dtype=torch.bool)
            conflict_timing_recon = _masked_mse(
                pred_state['conflict_timing_values'],
                clean_state['conflict_timing_values'],
                timing_valid_mask,
            )
        boundary_pred = self._stack_boundary_predictions(pred_state)
        boundary_recon = _masked_mse(
            boundary_pred,
            clean_state['boundary_values'],
            boundary_valid_mask,
        )
        total = (
            window_recon
            ...
            + temp_occ_recon
            + area_status_recon
            + conflict_timing_recon
            + boundary_recon
        )
        return {
            'state_diffusion_recon_loss': total,
            'state_window_recon_loss': window_recon,
            'state_dir_recon_loss': dir_recon,
            'state_decision_phase_recon_loss': decision_recon,
    ...
    def _compute_stage1_direct_losses(
        ...
        if conflict_area_result is not None:
            conflict_area_target, conflict_area_valid_mask = conflict_area_result
            ...
            conflict_area_valid_mask = conflict_area_valid_mask.to(device=device, dtype=torch.bool)
        ...
        if conflict_timing_targets is not None:
            if 'conflict_area_status_logits' not in raw_stage1_scores or 'conflict_timing_values' not in raw_stage1_scores:
                raise ValueError("conflict timing training requires model conflict_area_status_logits and conflict_timing_values")
            loss_conflict_area_status = F.cross_entropy(
                raw_stage1_scores['conflict_area_status_logits'].float(),
                conflict_timing_targets['status_target'],
            )
            timing_valid = conflict_timing_targets['timing_valid_mask'].to(device=device, dtype=torch.bool)
            if timing_valid.any():
                loss_conflict_timing = F.smooth_l1_loss(
                    raw_stage1_scores['conflict_timing_values'][timing_valid].float(),
                    conflict_timing_targets['timing_values'][timing_valid].float(),
                )
    ...
    def _compose_stage1_speed_energy_outputs(self, raw_scores: dict) -> dict:
        ...
        conflict_timing_values = raw_scores.get('conflict_timing_values', None)
        ...
        if conflict_area_status_probs is not None:
            composed['conflict_area_status_probs'] = conflict_area_status_probs
        if conflict_timing_values is not None:
            composed['conflict_timing_values'] = conflict_timing_values
            composed['conflict_dist_to_entry_m'] = (
                conflict_timing_values[:, 0] * float(self.conflict_timing_dist_norm_scale)
            )
            composed['conflict_dist_to_exit_m'] = (
                conflict_timing_values[:, 1] * float(self.conflict_timing_dist_norm_scale)
            )
            composed['conflict_time_to_entry_s'] = (
    ...
    def _compute_shared_stage1_speed_energy_loss(
        ...
        return self._compute_stage1_direct_losses(
            batch=batch,
            raw_stage1_scores=raw_stage1_scores,
            device=device,
            model_dtype=model_dtype,
        )
    ...
    def compute_split_loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.use_joint_state_diffusion:
            if not has_stage1_speed_energy:
                raise ValueError("joint state diffusion requires stage1 labels in the training batch")
            joint_state_clean, joint_state_metadata = self._build_joint_state_clean_targets(
                batch=batch,
            ...
            noisy_state = self._add_noise_to_state_dict(
                clean_state=joint_state_clean,
                timesteps=diff_timesteps,
            )
        elif self.use_traj_branch_condition and has_stage1_speed_energy:
        ...
        if train_stage1_speed_energy_active:
            ...
            if self.use_joint_state_diffusion:
                stage1_extra_losses.update({
                    'state_diffusion_recon_loss': state_recon_loss_dict.get('state_diffusion_recon_loss', zero_t),
                    'state_window_recon_loss': state_recon_loss_dict.get('state_window_recon_loss', zero_t),
                    'state_dir_recon_loss': state_recon_loss_dict.get('state_dir_recon_loss', zero_t),
                    'state_decision_phase_recon_loss': state_recon_loss_dict.get('state_decision_phase_recon_loss', zero_t),
    ...
    def _init_joint_state_noise(
        ...
        state = {
            ...
            'control_phase_logits': torch.randn(batch_size, 4, device=device, dtype=torch.float32),
            'conflict_area_logits': torch.randn(batch_size, self.num_waypoints, device=device, dtype=torch.float32),
        ...
        if self.use_conflict_timing_state:
            state['conflict_area_status_logits'] = torch.randn(batch_size, 4, device=device, dtype=torch.float32)
            state['conflict_timing_values'] = torch.randn(batch_size, 3, device=device, dtype=torch.float32)
        return state
    ...
    def _conditional_sample_joint_state(
        ...
        state_t = self._init_joint_state_noise(B, device)
        ...
        for step_idx, k in enumerate(roll_timesteps):
            ...
            poses_reg, route_pred, _, _, speed_pred, speed_profile_pred, state_pred_dict = self.model.forward_ego_joint(
                x_t=x_input,
                x_t_abs=x_t_abs,
                timestep=t_tensor,
                transfuser_bev_feature=transfuser_bev_feature,
            ...
            if use_guidance:
                ...
                with torch.enable_grad():
                    ...
                    if total_energy.requires_grad and pred_x0_for_grad.requires_grad:
                        ...
                        grad = grad * torch.clamp(self.energy_grad_clip_norm / grad_norm, max=1.0)
                    else:
                        grad = torch.zeros_like(pred_x0_for_grad)

                pred_joint_corrected = torch.cat(
                    [pred_x0.float() - self.guidance_scale * grad, route_pred_norm.float()],
                    dim=2,
                )
            else:
                energy_scores = None
                pred_joint_corrected = torch.cat(
                    [poses_reg.float(), route_pred.unsqueeze(1).float()],
                    dim=2,
                )
        ...
        final_joint_abs = self.joint_norm_to_abs(pred_joint_corrected)
        final_traj_abs = final_joint_abs[:, :, :self.horizon, :]
        route_pred = final_joint_abs[:, 0, self.horizon:, :]
        best_trajectory = final_traj_abs.squeeze(1)

        target_speed_pred = None
        if speed_pred is not None:
            target_speed_pred = self.decode_speed_two_hot(speed_pred, self.model.speed_classes)

        speed_energy_scores = None
        speed_energy_query_center = ego_status[:, -1, 0].to(device=device, dtype=model_dtype)
        speed_energy_ref_speeds = None
        speed_energy_ref_scores = None
        traj_branch_condition_probs = None
        traj_branch_condition_details = None
        if final_state_scores is not None:
            speed_energy_scores = self._compose_stage1_speed_energy_outputs(final_state_scores)
            branch_speed_ref = (
                target_speed_pred if target_speed_pred is not None
                else speed_energy_query_center
            )
            ...
            speed_energy_ref_speeds = torch.stack(
                [branch_speed_ref, traj_speed_1s_ref, traj_speed_05s_ref], dim=-1
            ).clamp_(0.0, 20.0)

        return {
            'best_trajectory': best_trajectory,
            'route_pred': route_pred,
            'all_trajectories': final_traj_abs,
            ...
            'conflict_area_status_probs': (
                speed_energy_scores.get('conflict_area_status_probs')
                if speed_energy_scores is not None else None
            ),
            'conflict_timing_values': (
                speed_energy_scores.get('conflict_timing_values')
                if speed_energy_scores is not None else None
            ),
            'conflict_dist_to_entry_m': (
                speed_energy_scores.get('conflict_dist_to_entry_m')
                if speed_energy_scores is not None else None
            ),
            'conflict_dist_to_exit_m': (
                speed_energy_scores.get('conflict_dist_to_exit_m')
                if speed_energy_scores is not None else None
            ),
            'conflict_time_to_entry_s': (
                speed_energy_scores.get('conflict_time_to_entry_s')
                if speed_energy_scores is not None else None
            ),
            ...
            'target_speed': target_speed_pred,
    ...
    def conditional_sample(
        ...
        """
        if self.use_joint_state_diffusion:
            return self._conditional_sample_joint_state(
                transfuser_bev_feature=transfuser_bev_feature,
                transfuser_bev_feature_upsample=transfuser_bev_feature_upsample,
                ego_status=ego_status,
                device=device,
