...
class DiffusionDiTCarlaPolicy(nn.Module):
    ...
    def conditional_sample(self,
        ...
        if no_noise:
            ...
            # ========== Standard DDIM or server-style iterative denoising ==========
            # Normalize delta to [-1, 1] for diffusion operations
            all_anchors_delta_normed = self.norm_delta(all_anchors_delta)  # (B, M, T, 2)

            if use_server_style:
                #print("Using server-style iterative forward (no DDIM step)")
                # Server-style: iterative forward, re-noise each step (no DDIM step)
                trunc_ts = torch.full((bs,), self.trunc_timesteps, dtype=torch.long, device=device)
                if server_noise_type == "multiplicative":
                    flat = all_anchors_delta_normed.view(bs * self.num_modes, horizon, 2)
                    trunc_ts_exp = trunc_ts.unsqueeze(1).expand(-1, self.num_modes).reshape(-1)
                    x_t = self.add_multiplicative_noise_scheduled_batch(
                        flat, timesteps=trunc_ts_exp, eta=self.diffusion_eta
                    ).view(bs, self.num_modes, horizon, 2)
                else:
                ...
                # ===== Diagnostic tracking for server-style inference =====
                prev_poses_reg_normed = None
                prev_best_mode_idx = None
                diag_logs = []
                ...
                for i in range(len(roll_timesteps) - 1):
                    ...
                    # ===== Diagnostic: track mode selection and per-mode diffs =====
                    cur_best_mode_idx = torch.argmax(poses_cls, dim=-1)  # (B,)
                    step_log = {
                        'step': i,
                        't_cur': t_cur,
                        't_next': t_next,
                        'best_mode_idx': cur_best_mode_idx.cpu().tolist(),
                    }

                    if prev_poses_reg_normed is not None:
                        # Per-mode mean abs diff: (B, M, T, 2) -> mean over (T, 2) -> (B, M)
                        per_mode_diff = (poses_reg_normed - prev_poses_reg_normed).abs().mean(dim=(-2, -1))  # (B, M)

                        # Best mode diff (using current step's best mode)
                        best_diff = per_mode_diff[torch.arange(bs), cur_best_mode_idx]  # (B,)
                        # Other modes diff: mask out best, average rest
                        mask = torch.ones_like(per_mode_diff, dtype=torch.bool)
                        mask[torch.arange(bs), cur_best_mode_idx] = False
                        other_diff = per_mode_diff[mask].view(bs, -1).mean(dim=-1)  # (B,)

                        step_log['best_mode_diff_mean'] = best_diff.abs().mean().item()
                        step_log['other_mode_diff_mean'] = other_diff.abs().mean().item()
                        step_log['per_mode_diff'] = per_mode_diff.abs().mean(dim=0).cpu().tolist()  # (M,) averaged over batch
                        step_log['mode_changed'] = (cur_best_mode_idx != prev_best_mode_idx).cpu().tolist()
                        step_log['mode_change_count'] = (cur_best_mode_idx != prev_best_mode_idx).sum().item()

                    diag_logs.append(step_log)
                    prev_poses_reg_normed = poses_reg_normed.detach().clone()
                    prev_best_mode_idx = cur_best_mode_idx.clone()

                    if i < len(roll_timesteps) - 1:
                        t_next_tensor = torch.full((bs,), t_next, dtype=torch.long, device=device)

                        # ===== Fix OOD: reset non-best modes to anchor + fresh noise =====
                        # Only keep best mode's prediction; other modes reload from anchor
                        best_idx = cur_best_mode_idx  # (B,)
                        # Start from anchor for all modes
                        next_input = all_anchors_delta_normed.clone()  # (B, M, T, 2)
                        # Overwrite best mode with model prediction
                        for b in range(bs):
                            next_input[b, best_idx[b]] = poses_reg_normed[b, best_idx[b]]

                        if server_noise_type == "multiplicative":
                            flat_pred = next_input.view(bs * self.num_modes, horizon, 2)
                            t_next_exp = t_next_tensor.unsqueeze(1).expand(-1, self.num_modes).reshape(-1)
                            x_t = self.add_multiplicative_noise_scheduled_batch(
                                flat_pred, timesteps=t_next_exp, eta=self.diffusion_eta
                            ).view(bs, self.num_modes, horizon, 2)
                        else:
                            noise = torch.randn_like(next_input)
                            x_t = self.diffusion_scheduler.add_noise(
                                original_samples=next_input,
                                noise=noise,
                                timesteps=t_next_tensor
                            )
                    else:
                        x_t = poses_reg_normed

                # ===== Print diagnostic summary =====
                print("\n========== Server-style Inference Diagnostics ==========")
                for log in diag_logs:
                    print(f"  Step {log['step']}: t_cur={log['t_cur']}, t_next={log['t_next']}")
                    print(f"    best_mode_idx = {log['best_mode_idx']}")
                    if 'best_mode_diff_mean' in log:
                        print(f"    mode_changed = {log['mode_changed']} (count={log['mode_change_count']})")
                        print(f"    best_mode_diff (mean) = {log['best_mode_diff_mean']:.6f}")
                        print(f"    other_mode_diff (mean) = {log['other_mode_diff_mean']:.6f}")
                        print(f"    ratio (other/best) = {log['other_mode_diff_mean'] / max(log['best_mode_diff_mean'], 1e-10):.2f}x")
                        diffs = log['per_mode_diff']
                        top5 = sorted(enumerate(diffs), key=lambda x: x[1], reverse=True)[:5]
                        print(f"    top-5 changed modes: {[(idx, f'{v:.4f}') for idx, v in top5]}")
                print("========================================================\n")
                ...
                # ===== Prepare GT and anchor in normed space for diagnostic =====
                gt_delta_normed = None
                anchor_normed = self.norm_delta(
                    self.anchor_centers.unsqueeze(0).expand(bs, -1, -1, -1).to(device=device, dtype=x_t.dtype)
                )  # (B, M, T, 2)
                if gt_trajectory is not None:
                    gt_delta = self._traj_to_delta(gt_trajectory.to(device=device, dtype=x_t.dtype))  # (B, T, 2)
                    gt_delta_normed = self.norm_delta(gt_delta)  # (B, T, 2)
                ...
                for i in range(len(roll_timesteps) - 1):
                    ...
                    # ===== Diagnostic: pred_x0 quality per mode =====
                    best_idx = torch.argmax(poses_cls, dim=-1)  # (B,)
                    ...
                    print(f"\n  [DDIM step {i}] t={t_cur}->{t_next}, sqrt(1-alpha_t)={(1-alpha_t).sqrt().item():.4f}")
                    for b in range(min(bs, 2)):
                        bi = best_idx[b].item()
                        print(f"    batch {b}: best_mode={bi}, cls_top3={torch.topk(poses_cls[b], 3).indices.tolist()}")

                        if gt_delta_normed is not None:
                            gt_n = gt_delta_normed[b]  # (T, 2)
                            # anchor vs GT in normed space (the root cause of pred_eps blow-up)
                            anchor_gt_dist = (anchor_normed[b] - gt_n.unsqueeze(0)).abs().mean(dim=(-2, -1))  # (M,)
                            # pred_x0 vs GT
                            pred_error_per_mode = (pred_x0_normed[b] - gt_n.unsqueeze(0)).abs().mean(dim=(-2, -1))  # (M,)
                            # x_t vs anchor: how far has x_t drifted from anchor? (OOD indicator at step>0)
                            x_t_anchor_dist = (x_t[b] - anchor_normed[b]).abs().mean(dim=(-2, -1))  # (M,)
                            # Physical L2
                            pred_x0_delta = self.denorm_delta(pred_x0_normed[b])  # (M, T, 2)
                            pred_x0_abs = torch.cumsum(pred_x0_delta, dim=-2)  # (M, T, 2)
                            gt_abs = gt_trajectory[b].to(device=device, dtype=x_t.dtype)  # (T, 2)
                            l2_per_mode = (pred_x0_abs - gt_abs.unsqueeze(0)).norm(dim=-1).mean(dim=-1)  # (M,)
                            print(f"      anchor vs GT (normed MAE):  best={anchor_gt_dist[bi]:.4f}, "
                                  f"others_mean={anchor_gt_dist.sum().sub(anchor_gt_dist[bi]).div(self.num_modes-1):.4f}")
                            print(f"      pred_x0 vs GT (normed MAE): best={pred_error_per_mode[bi]:.4f}, "
                                  f"others_mean={pred_error_per_mode.sum().sub(pred_error_per_mode[bi]).div(self.num_modes-1):.4f}")
                            print(f"      pred_x0 vs GT (L2 meters):  best={l2_per_mode[bi]:.4f}, "
                                  f"others_mean={l2_per_mode.sum().sub(l2_per_mode[bi]).div(self.num_modes-1):.4f}")
                            print(f"      x_t vs anchor (normed MAE): best={x_t_anchor_dist[bi]:.4f}, "
                                  f"others_mean={x_t_anchor_dist.sum().sub(x_t_anchor_dist[bi]).div(self.num_modes-1):.4f}"
                                  f"  <- {'~noise level' if i == 0 else 'OOD if >> noise level'}")

                        # pred_eps magnitude (should be ~1.0 for well-calibrated model)
                        pred_eps_mag = pred_eps[b].abs().mean(dim=(-2, -1))  # (M,)
                        print(f"      pred_eps_mag (should~1.0): best={pred_eps_mag[bi]:.4f}, "
                              f"others_mean={pred_eps_mag.sum().sub(pred_eps_mag[bi]).div(self.num_modes-1):.4f}, "
        ...
        # Select best mode using classification scores from the last forward pass
