...
def diagnose_lidar_format(lidar_raw, frame_num=0, verbose=True):
	"""
	诊断 CARLA LiDAR 点云的格式和分布
	
	Args:
		lidar_raw: CARLA LiDAR 原始数据
		frame_num: 帧号，用于日志输出
		verbose: 是否打印详细信息
	"""
	if not verbose:
	...
	try:
		if not isinstance(lidar_raw, np.ndarray):
			lidar_raw = np.array(lidar_raw)
		
		print(f"\n{'='*70}")
		print(f"LiDAR 数据诊断 (Frame {frame_num})")
		print(f"{'='*70}")
		print(f"数据格式:")
		print(f"  - 总点数: {len(lidar_raw)}")
		print(f"  - 数据形状: {lidar_raw.shape}")
		print(f"  - 数据类型: {lidar_raw.dtype}")
		
		if lidar_raw.shape[1] == 3:
			print(f"  - 通道: (X, Y, Z) - 缺少 Intensity 通道")
		elif lidar_raw.shape[1] == 4:
			print(f"  - 通道: (X, Y, Z, Intensity)")
		else:
			print(f"  - 通道: 未知格式 ({lidar_raw.shape[1]} 通道)")
		
		print(f"\n坐标范围 (原始):")
		print(f"  - X: [{lidar_raw[:, 0].min():8.2f}, {lidar_raw[:, 0].max():8.2f}]")
		print(f"  - Y: [{lidar_raw[:, 1].min():8.2f}, {lidar_raw[:, 1].max():8.2f}]")
		print(f"  - Z: [{lidar_raw[:, 2].min():8.2f}, {lidar_raw[:, 2].max():8.2f}]")
		
		if lidar_raw.shape[1] >= 4:
			print(f"  - Intensity: [{lidar_raw[:, 3].min():8.2f}, {lidar_raw[:, 3].max():8.2f}]")
		
		# 分析坐标分布
		x_center = (lidar_raw[:, 0].min() + lidar_raw[:, 0].max()) / 2
		y_center = (lidar_raw[:, 1].min() + lidar_raw[:, 1].max()) / 2
		
		print(f"\n坐标分布:")
		print(f"  - X 中心: {x_center:8.2f}")
		print(f"  - Y 中心: {y_center:8.2f}")
		
		# 象限统计
		quad_pp = ((lidar_raw[:, 0] > 0) & (lidar_raw[:, 1] > 0)).sum()
		quad_mp = ((lidar_raw[:, 0] < 0) & (lidar_raw[:, 1] > 0)).sum()
		quad_mm = ((lidar_raw[:, 0] < 0) & (lidar_raw[:, 1] < 0)).sum()
		quad_pm = ((lidar_raw[:, 0] > 0) & (lidar_raw[:, 1] < 0)).sum()
		
		print(f"\n象限分布 (X正Y正/X负Y正/X负Y负/X正Y负):")
		print(f"  - {quad_pp:6d} / {quad_mp:6d} / {quad_mm:6d} / {quad_pm:6d}")
		
		print(f"\nBOUNDARY 对比:")
		print(f"  - BOUNDARY X: [{BOUNDARY['minX']}, {BOUNDARY['maxX']}]")
		print(f"  - BOUNDARY Y: [{BOUNDARY['minY']}, {BOUNDARY['maxY']}]")
		print(f"  - BOUNDARY Z: [{BOUNDARY['minZ']}, {BOUNDARY['maxZ']}]")
		
		# 检查边界覆盖
		x_coverage = (
			(lidar_raw[:, 0] >= BOUNDARY['minX']).sum(),
			(lidar_raw[:, 0] <= BOUNDARY['maxX']).sum()
		)
		y_coverage = (
			(lidar_raw[:, 1] >= BOUNDARY['minY']).sum(),
			(lidar_raw[:, 1] <= BOUNDARY['maxY']).sum()
		)
		z_coverage = (
			(lidar_raw[:, 2] >= BOUNDARY['minZ']).sum(),
			(lidar_raw[:, 2] <= BOUNDARY['maxZ']).sum()
		)
		
		print(f"\n边界内点数:")
		print(f"  - X 在范围内: {x_coverage[0]} ({100*x_coverage[0]/len(lidar_raw):.1f}%)")
		print(f"  - Y 在范围内: {y_coverage[0]} ({100*y_coverage[0]/len(lidar_raw):.1f}%)")
		print(f"  - Z 在范围内: {z_coverage[0]} ({100*z_coverage[0]/len(lidar_raw):.1f}%)")
		
		# 三维边界内
		in_boundary = (
			(lidar_raw[:, 0] >= BOUNDARY['minX']) & (lidar_raw[:, 0] <= BOUNDARY['maxX']) &
			(lidar_raw[:, 1] >= BOUNDARY['minY']) & (lidar_raw[:, 1] <= BOUNDARY['maxY']) &
			(lidar_raw[:, 2] >= BOUNDARY['minZ']) & (lidar_raw[:, 2] <= BOUNDARY['maxZ'])
		).sum()
		
		print(f"  - 三维都在范围内: {in_boundary} ({100*in_boundary/len(lidar_raw):.1f}%)")
		print(f"{'='*70}\n")
		
	except Exception as e:
		print(f"诊断出错: {e}", flush=True)