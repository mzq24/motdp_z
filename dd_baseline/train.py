#!/usr/bin/env python3
"""
Training script for DD Baseline (DiffusionDrive reproduction).

Reuses MoT-DP's distributed training infrastructure, dataset, and dataloader.
Only changes the policy model to DDBaselinePolicy.

Usage:
    # Single GPU
    python dd_baseline/train.py --config_path dd_baseline/dd_config.yaml

    # Multi-GPU (torchrun)
    torchrun --nproc_per_node=2 dd_baseline/train.py --config_path dd_baseline/dd_config.yaml
"""
import os
import sys
import torch
...
from dd_baseline.policy import DDBaselinePolicy
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with autocast('cuda', enabled=use_amp, dtype=amp_dtype):
                # Temporarily set train mode to get loss (forward_test doesn't compute loss)
...
@record
def train_dd_baseline(config_path, resume_path=None, val_only=False):
    ...
    if rank == 0:
        print(f'[DD Baseline] Rank: {rank}, Device: {device}, World size: {world_size}')
    ...
    # Inject val features from train's memmap into RAM (skip if using per-frame loading)
    if not use_per_frame:
        _max_val_per_rank = val_max_batches * val_batch_size if val_max_batches else None
        val_dataset.inject_ram_features(train_dataset, rank=rank, world_size=world_size, max_val_samples=_max_val_per_rank)
    ...
    # Model
    if rank == 0:
        print("Initializing DD Baseline policy...")
    policy = DDBaselinePolicy(config).to(device)
    if rank == 0:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if rank == 0 and (epoch + 1) % save_freq == 0:
            ckpt_path = os.path.join(checkpoint_dir, f"dd_baseline_epoch{epoch+1}.pt")
            torch.save({
            ...
            periodic_ckpts = sorted(
                glob_module.glob(os.path.join(checkpoint_dir, "dd_baseline_epoch*.pt")),
            ...
            if rank == 0:
                ...
                if l2_avg < best_l2_avg:
                    ...
                    torch.save({
                        ...
                        'val_loss': val_loss, 'train_loss': avg_train_loss,
                        'val_metrics': val_metrics,
                    }, os.path.join(checkpoint_dir, "dd_baseline_best.pt"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DD Baseline (DiffusionDrive reproduction)")
    parser.add_argument('--config_path', type=str,
                        default=os.path.join(os.path.dirname(__file__), 'dd_config.yaml'))
    parser.add_argument('--resume', type=str, default=None)
    parser.add_argument('--val_only', action='store_true')
    args = parser.parse_args()
    train_dd_baseline(config_path=args.config_path, resume_path=args.resume, val_only=args.val_only)
