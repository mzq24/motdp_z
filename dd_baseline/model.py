"""
DDBaselineModel: top-level model for DiffusionDrive baseline.

Adapts MoT-DP's pre-extracted BEV features to DiffusionDrive's TrajectoryHead:
1. Projects bev_feature (1512, 8, 8) + bev_feature_upsample (64, 64, 64) -> (256, 64, 64)
2. Encodes ego_status into query via standard TransformerDecoder
3. Provides learned agent queries (replacing agent detection head)
4. Runs TrajectoryHead for diffusion-based trajectory prediction
"""
import torch
...
class DDBaselineModel(nn.Module):
    def __init__(self, config: DDBaselineConfig):
        ...
        # Key-value embedding for standard transformer decoder
        # 8x8 BEV tokens + 1 status token = 65 tokens
    ...
    def forward(self, features: Dict[str, torch.Tensor],
        ...
        # 5. Build cross_bev_feature for GridSample attention
        # Upsample bev_down from 8x8 to 64x64, then concat with bev_upsample
